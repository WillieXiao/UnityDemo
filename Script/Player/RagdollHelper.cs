...
public class RagdollHelper : MonoBehaviour {
		...
		set{
			if (value==true){
				...
				if (state==RagdollState.ragdolled) {
                    ...
                    if (Physics.Raycast(anim.GetBoneTransform(HumanBodyBones.Hips).position, anim.GetBoneTransform(HumanBodyBones.Hips).forward, 0.3f,ground))
                    {
    ...
    public void GetUpRestore()
    {
        ...
        {
            agent.isStopped = false;
            if (armedAI.state == ArmedState.Fight)
            {
                armedAI.fighting = true;
            }
        }
    }
    private void OnDrawGizmos()
    {
        ...
        //Gizmos.DrawRay(anim.GetBoneTransform(HumanBodyBones.Hips).position, anim.GetBoneTransform(HumanBodyBones.Hips).forward * 0.2f);
    }
